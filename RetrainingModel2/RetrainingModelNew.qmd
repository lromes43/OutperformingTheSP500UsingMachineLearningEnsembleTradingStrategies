...
```{python}
data = pd.read_feather("/Users/lukeromes/Desktop/Personal/Sp500Project/RetrainingModel2/CHECKFEATHER.feather")
data['Date'] = data['Date'].dt.date
```

Just fitting model no test
...
X_train_raw = data.drop(drop_cols, axis=1)


X_train_final = pd.get_dummies(X_train_raw, drop_first=True)


y = data['Movement'].astype(int)


dtrain = xgb.DMatrix(X_train_final, label=y)
...
```{python}
num_boost_rounds = 26
best_gamma = 0.026010000000000002
tuned_depth = 7
tuned_min_child_weight = 2
...
```{python}
additional_params = {
    "subsample":np.arange(0.01, 1, .002),
    "colsample_bytree":np.arange(0.01, 1, .002)
}
...
```{python}
num_boost_rounds = 838
best_gamma = 0.1
tuned_depth = 7
tuned_min_child_weight = 10
tuned_subsample = 1.0
tuned_colsample = .9
scale_pos_weight = .9552332056610436
