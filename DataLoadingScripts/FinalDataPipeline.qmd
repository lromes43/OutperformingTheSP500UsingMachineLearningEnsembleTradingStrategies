...
def initial_data_loading(length, frequency):
   ...
   # ... (rest of initial_data_loading function)

    date_regex = re.compile(r"^\d{4}-\d{2}-\d{2}$")

    # Read the combined file. The first column is the Date column.
    # We drop the first (unnamed) column if you forgot index=False in the save step.
    df = pd.read_csv("/Users/lukeromes/Desktop/Personal/Sp500Project/combined_sp500_yearly_data.csv")
    
    # If the combined file was saved with the index, the Date column is the SECOND column, 
    # but the column *labels* start from 0 if you use header=None.

    # --- SIMPLIFIED AND CORRECTED CLEANING ---

    # 1. Ensure the DataFrame is properly formatted (assuming Date is the first column now)
    df.columns = ["Unnamed Index Column", "Date", "Open", "High", "Low", "Close", "Volume", "Ticker"]

    # 2. Drop the unnecessary 'Unnamed Index Column' created by the previous to_csv
    df = df.drop(columns=["Unnamed Index Column"])
    
    # 3. Apply the regex filter to the 'Date' column
    clean = df[df['Date'].astype(str).str.match(date_regex)]

    # 4. Save the cleaned data
    clean.to_csv("cleaned.csv", index=False)

    # 5. The subsequent read and column renaming is now redundant if the above is done correctly.
    # We can just use 'clean' as the final_df
    final_df = clean

    # Check the final columns before returning
...
```{python}
test = pd.read_feather("/Users/lukeromes/Desktop/Personal/Sp500Project/TEST.feather")
test
```
