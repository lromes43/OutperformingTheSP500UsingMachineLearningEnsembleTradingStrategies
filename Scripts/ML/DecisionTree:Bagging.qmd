...
```{python}
cols_to_drop = ['Date', 'Daily_Return','next_day_pct_change','next_5_day_pct_change','Movement_5_day','next_30_day_pct_change','Movement_30_day','Movement']
...
plt.title("Confusion Matrix for Initial Decison Tree")
plt.show()
```
...
filename_tree = 'decisiontree.pkl'
...
plt.show()
print(optimal_iterations_acc)

```
Getting 53.0% accuracy
...
```{python}
ypred_enc = tuned_bag.predict(X_test_final)
ypred = le.inverse_transform(ypred_enc)
...
plt.show()
print(FinalTunedBagged_acc)

```

Gives 0.4888 accuracy so worse than our final bagged, optimal iterations is the best option