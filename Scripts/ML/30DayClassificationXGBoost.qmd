...
y_train_final = train['Movement_30_day'].astype(int)
y_test_final  = test['Movement_30_day'].astype(int)
...
Getting 55.3% accuracy before any tuning
...
for g in tqdm(gamma, desc="Tuning gamma"):
...
subsamples = additional_params["subsample"]
colsamples = additional_params["colsample_bytree"]

for ss in tqdm(subsamples, desc="Tuning subsample"):
    for cst in tqdm(colsamples, desc=f"colsample_bytree for subsample={ss}", leave=False):
...
Accuracy goes up 3% to 56.3% after tuning

```{python}
import joblib
final_model_filename = 'BoostedThirtyDayClassifier.job.lib'
