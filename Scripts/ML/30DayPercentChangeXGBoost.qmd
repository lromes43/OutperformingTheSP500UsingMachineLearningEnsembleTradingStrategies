...
MSE for thirty day change: 0.011406
...
Can Predict the movement to the nearest .5% with .0485.
...
for i in range(1, num_rounds + 1):
    pred_i = model.predict(dtest, iteration_range=(0, i))
...
test2merged = test2merged.rename(columns = {'next_30_day_pct_change':'actual'})
```
...
Getting 248 for optimal iterations
...
```{python}
final30daypercentchange = FinalBoosted_30.predict(dtest)
merged['final_prediction'] = final30daypercentchange
```
...
MSE for one day change: 0.011227
...
```{python}
thirty_day_rmse_final = Final_MSE**.5
print(thirty_day_rmse_final)
```
...
```{python}
RMSE_Change = float(thirty_day_rmse - rmse)
RMSE_Change
```
...
```{python}
import joblib
final_model_filename = 'ContinuousThirtyDayFinal.joblib'
joblib.dump(FinalBoosted_30, final_model_filename)
```