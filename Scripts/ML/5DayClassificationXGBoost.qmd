...
y_train_final = train['Movement_5_day'].astype(int)
y_test_final  = test['Movement_5_day'].astype(int)
...
plt.title("Initial 5 Day Boosted Confusion Matrix")
plt.show()
...
```{python}
best_gamma = float(gamma_df.iloc[1]['gamma'])
```
...
watchlist = [(dtrain, "training")]
five_day_final_boosted=  xgb.train(
    params = final_params,
    dtrain = dtrain,
    num_boost_round = num_boost_rounds,
    evals = watchlist, 
...
```{python}
test_pred_tuned = five_day_final_boosted.predict(dtest)
test_pred_classification = (test_pred_tuned >=.5).astype(int)

cm = confusion_matrix(y_test_final, test_pred_classification)
five_day_final_boostedcm = ConfusionMatrixDisplay(confusion_matrix=cm)
five_day_final_boostedcm.plot(cmap = "Blues")
plt.title("Final 5 Day Binary Boosted Confusion Matrix")
plt.show()
final_boosted_acc = accuracy_score(y_test_final, test_pred_classification)
print(final_boosted_acc)
```

Getting 69.73% for 5 day Prediction.



```{python}
final_model_filename = 'FinalBoostedFiveDayClassifier.job.lib'
joblib.dump(five_day_final_boosted, final_model_filename)
```