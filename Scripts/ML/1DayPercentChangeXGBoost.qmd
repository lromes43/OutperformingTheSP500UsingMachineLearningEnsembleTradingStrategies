...
MAE_df = pd.DataFrame(y_pred_one).reset_index()
...
merged_MAE_df_one = merged_MAE_df
```
...
```{python}
error = []
for i in range(0, len(merged_MAE_df_one+1)):
    diff = float(merged_MAE_df_one['Actual'].iloc[i] - merged_MAE_df_one['Predicted'].iloc[i])
    error.append(diff)
    MAE = (1/len(merged_MAE_df_one)) * sum(abs(merged_MAE_df_one['Actual'] - merged_MAE_df_one['Predicted']))

print(f"MAE for 1 Day Change: {MAE}")

```

MAE for one day change: 0.0195218
...
for i in range(0, len(merged_MAE_df+1)):
    diff = float(merged_MAE_df_one['Actual'].iloc[i] - merged_MAE_df_one['Predicted'].iloc[i]) **2
    error.append(diff)
    MSE = (1/len(merged_MAE_df_one)) * sum(error)
```

MSE for one day change: 0.007533

R-Squared


```{python}
r_squared_one_day_change = r2_score(merged_MAE_df_one['Actual'], merged_MAE_df_one['Predicted'])
```

R-Squared for One-Day: -76.6996%


```{python}
threshold = 0.005
close_accuracy = np.mean(np.abs(y_pred_one - y_test_final) <= threshold)
print("Within ±0.5% accuracy:", close_accuracy)

```

Can Predict the movement to the nearest .5% with 27.5% accuracy.