...
MAE_df = pd.DataFrame(y_pred).reset_index()
...
for i in range(0, len(merged_MAE_df+1)):
    diff = float(merged_MAE_df['Actual'].iloc[i] - merged_MAE_df['Predicted'].iloc[i])
    error.append(diff)
    MAE = (1/len(merged_MAE_df)) * sum(error)
...
error = []
for i in range(0, len(merged_MAE_df+1)):
    diff = float(merged_MAE_df['Actual'].iloc[i] - merged_MAE_df['Predicted'].iloc[i]) **2
    error.append(diff)
    MSE = (1/len(merged_MAE_df)) * sum(error)
```
