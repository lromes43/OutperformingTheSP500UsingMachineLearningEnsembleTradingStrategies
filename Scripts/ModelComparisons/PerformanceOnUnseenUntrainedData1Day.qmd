...
actual = transformed_data['Movement'].astype(int)
...
```{python}
merged_binary_one.to_csv("binaryoneunseen.csv")
```
...
y_final = subset2['next_day_pct_change'].astype(float)
...
```{python}
merged_cont_one.to_csv("contoneunseen.csv")
```
...
MSE: 4.1633584312570635
MSE_percent: 416.33584312570633
 one day rmse: 2.040430942535685
one day rmse percent: 204.04309425356848
Within ±0.5% accuracy (in percentage points): 0.2434077079107505
