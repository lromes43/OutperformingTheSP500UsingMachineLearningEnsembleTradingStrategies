...
format: html
---
Want to see how model preforms on unseen data it is not trained on at all! SO i pulled nov 11 - nov 28
...
Load in data and preform the function preprocess to create all variables
```{python}
data = pd.read_feather("/Users/lukeromes/Desktop/Personal/Sp500Project/Data/FinalTestDataForPipeline.feather")
...
def preprocess (data):
    ...
    data.rename(columns={'VIX_Close': '^VIX'}, inplace=True)
    ...
    for ticker in data['Ticker'].unique():
        ...
        for i in range(0, len(subset)):
            if i == len(subset)-1:
                ...
                diff_val = ((price_tom - price_today) / price_today)*100  # percent change
    ...
    final_data = data
    final_data.to_feather('DerivedAttributes.feather')

    final_data 

preprocess(data)
transformed_data = pd.read_feather("/Users/lukeromes/Desktop/Personal/Sp500Project/DerivedAttributes.feather")

```


First Day not in model, training or validation is 2025-11-11 so will subset that so can see how one day binary and continuous preform
```{python}
transformed_data = pd.read_feather("/Users/lukeromes/Desktop/Personal/Sp500Project/DerivedAttributes.feather")

transformed_data = transformed_data.rename(columns={'Two_Week_SD': 'Two_Week_SD '}) 
```
...
drop_cols = ['Date', 'next_day_pct_change',
 'next_5_day_pct_change',
 'Movement_5_day',
 'next_30_day_pct_change',
 'Movement_30_day',
...
X = transformed_data.drop(drop_cols, axis = 1)
X_final = pd.get_dummies(X, drop_first=True)
actual = transformed_data['Movement_5_day'].astype(int)
...
merged_binary_five = merged_binary

```

```{python}
merged_binary_five.to_csv("binary5unseen.csv")
```
...
y_final = transformed_data['next_5_day_pct_change'].astype(float)
...
sorted_ticker_series = subset2['Ticker'].sort_values(ascending=True)
merged['ticker'] = sorted_ticker_series.reset_index(drop=True)
merged_cont_five = merged 
merged_cont_five = merged_cont_five.rename(columns = {'next_5_day_pct_change': 'actual'})
```

```{python}
merged_cont_five.to_csv("cont5unseen.csv")
```


```{python}

squared_errors = (merged_cont_five['actual'] - merged_cont_five['Initial_Predicted']) ** 2
...
threshold = 0.5 
close_accuracy = np.mean(np.abs(merged_cont_five['actual'] - merged_cont_five['Initial_Predicted']) <= threshold)
print(f"Within ±0.5% accuracy (in percentage points): {close_accuracy}")
```

MSE: 17.683659270995815
MSE_percent: 1768.3659270995815
 one day rmse: 4.205194320242028
one day rmse percent: 420.5194320242028
Within ±0.5% accuracy (in percentage points): 0.07475031210986267
