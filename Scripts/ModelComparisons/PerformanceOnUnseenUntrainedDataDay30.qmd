...
actual = transformed_data['Movement_30_day'].astype(int)
...
```{python}
merged_binary_thirty.to_csv("binary30unseen.csv")
```
...
```{python}
y_pred_thirty = continuousone.predict(dtest)
merged_cont_thirty = pd.DataFrame(merged_cont_thirty)
y_test_new = y_test_new.reset_index().drop('index', axis =1).rename(columns = {'level_0':'iteration', 'next_day_pct_change': 'actual'})
cols = y_test_new.columns.tolist()

cols_to_keep = ['iteration', 'next_30_day_pct_change']
y_test_new_cleaned = y_test_new[cols_to_keep].iloc[:, [0, -1]]

y_test_new_cleaned = y_test_new.iloc[:, [0, 3]].copy()
y_test_new_cleaned.columns = ['iteration', 'actual'] 
MAE_df = pd.DataFrame(y_pred_one).reset_index().rename(columns = {'index': 'iteration', 0 : 'Initial_Predicted'})

merged = pd.merge(y_test_new_cleaned, MAE_df, how = 'inner', on = 'iteration')
...
merged_cont_thirty = merged_cont_thirty.rename(columns = {'next_5_day_pct_change': 'actual'})
```

Cannot make predictions yet due to it month not being here.
